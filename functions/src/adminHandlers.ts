...
export const adminRefundOrder = onCall(
  { region: "asia-east1", maxInstances: 3, secrets: PAYPAL_SECRETS },
  async (request) => {
...
export const adminAdjustCoins = onCall({ region: "asia-east1", maxInstances: 3 }, async (request) => {
  const operatorUid = await assertAdmin(request.auth?.uid);
...
export const adminSetPaymentReview = onCall({ region: "asia-east1", maxInstances: 3 }, async (request) => {
  const operatorUid = await assertAdmin(request.auth?.uid);
 ...
 */
export const adminReconcile = onCall({ region: "asia-east1", maxInstances: 1, timeoutSeconds: 120 }, async (request) => {
